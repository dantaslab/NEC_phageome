...
df_meds = read_excel("sample_medications_DOL.xlsx")
sample_row_name = df_meds$DNA_Sample_ID
df_meds = df_meds[,-1]
row.names(df_meds) = sample_row_name

for (i in 1:dim(df_meds)[2]){
  sample_data<- df_meds[,i]
...
write.table(PERMANOVA_result[1,], "PERMANOVA_result_phage_vOTU_medications.csv", 
