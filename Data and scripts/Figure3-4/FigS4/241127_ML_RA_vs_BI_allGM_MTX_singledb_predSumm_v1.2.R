...
all.pred.Val2.RA <- read.csv('241127_ML_allTaxa_MTX_singledb_pred_v1.2.csv', header = T)
all.pred.Val2.RA$Group = "RA"
all.pred.Val2.BI <- read.csv('241127_ML_allTaxa_MTX_singledb_pred_v1.2_BI.csv', header = T)
all.pred.Val2.BI$Group = "BI"
all.pred.Val2 = rbind(all.pred.Val2.RA, all.pred.Val2.BI)
...
pred_vars <- c('Balanced.Accuracy', 'Sensitivity', 'Specificity')
               # 'F1','Recall','Precision')
fillcols <- c('#f0b858', '#5c6a87') 
linecols <- c('#1f2e2e','#1f2e2e')
...
p_rf_pred_box <- ggplot(all.pred.long, aes(x=DataSet, y=Performance_Measure_Value, 
  ...
  stat_compare_means(method = "wilcox.test", label ="p.signif", label.y = 1.05, vjust = 1) + #p.format p.signif
  # scale_x_discrete(limits = rev)+
  ylim(0,NA)+   
  coord_flip()
p_rf_pred_box
ggsave("241127_box_Pred_all_vs_select_GM_singledb_DBO>0_v1.2.pdf", p_rf_pred_box , width=10, height=3)



all.pred = filter(all.pred.Val2, DataSet == "BacTaxa") ###############################
...
all.pred.long$Performance_Measure <- factor(all.pred.long$Performance_Measure,
                                            levels = c("Balanced.Accuracy","Sensitivity","Specificity"))
all.pred.long$Performance_Measure_Value <- as.numeric(all.pred.long$Performance_Measure_Value)
# all.pred.long$Days_before_onset <- as.factor(all.pred.long$Days_before_onset)
...
for (tp in 0:8) {
  for (measure in c('Balanced.Accuracy', 'Sensitivity', 'Specificity')){
    ...
    t_test_result <- all.pred.long_tp %>% pairwise_t_test(Performance_Measure_Value ~ Group, ref.group = "RA", p.adjust.method = "BH")
    t_test_result$Days_before_onset = tp
    t_test_result$Performance_Measure = measure
    t_test_result$mean_Performance_Measure = c(mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$Group=='BI']))
...
p_rf_pred_line 
ggsave("241111_box_Pred_allbacTaxa_vs_selectbacTaxa_DBO_v1.2_class.pdf", p_rf_pred_box , width=8, height=2.5)
