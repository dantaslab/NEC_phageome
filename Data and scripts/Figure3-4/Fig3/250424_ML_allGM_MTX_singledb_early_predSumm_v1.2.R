...
all.pred.Val2 <- read.csv('250424_ML_allTaxa_MTX_singledb_early_pred_v1.2.csv', header = T)
...
mu <- ddply(subset(all.pred.long, DataSet == "VirTaxa"), #VirTaxa
...
p_rf_pred_box
ggsave("250424_box_Pred_allTaxa_MTX_early_singledb_DBO>0_v1.2.pdf", p_rf_pred_box , width=10, height=3)
...
p_rf_pred_line 
ggsave("250424_line_Pred_allTaxa_MTX_early_singledb_DBO_v1.2.pdf", p_rf_pred_line , width=12, height=3)
...
ggheatmap_sig
ggsave("250424_heatmap_Pred_allTaxa_MTX_early_singledb_DBO_v1.2.pdf.pdf", ggheatmap_sig, width=12, height=3)

all.pred.long.vir = filter(all.pred.long, DataSet == "VirTaxa")
p_rf_pred_line_vir <- ggplot(all.pred.long.vir, aes(x=Days_before_onset, y=Performance_Measure_Value, 
  ...
  scale_x_reverse(limits = c(8, 0), breaks=c(0:8))
p_rf_pred_line_vir
ggsave("250424_lineVir_Pred_allTaxa_MTX_early_singledb_DBO_v1.2.pdf", p_rf_pred_line_vir , width=10, height=3)
