...
SM_DF <- read.csv('NEC_bacteria_species_RA.csv',
                  header = T,
                  row.names = 1)
...
colnames(taxa_rich)[1] <- "Richness_bacSpecies"
...
plot_richness_DOL <- ggplot(taxa_rich_meta, aes(x=DOL_Group, y=Richness_bacSpecies, fill=NEC_status)) +
  geom_boxplot() +
  ...
  ylab("Bacteria Species Richness") +
...
plot_richness_DOL
ggsave("Box_Richness_bacSpecies_DOL.pdf", plot_richness_DOL, width=7, height=6)


##--------------------------------------------------------------------------------------------------------
## Calculate and compare SD
taxa_SD <- as.data.frame(diversity(SM_DF, index="shannon"))
colnames(taxa_SD)[1] <- "ShannonDiversity_BacSpecies"
...
plot_SD_DOL <- ggplot(taxa_SD_meta, aes(x=DOL_Group, y=ShannonDiversity_BacSpecies, fill=NEC_status)) +
  geom_boxplot() +
  scale_fill_manual(values=c("case"="#f28a5f","control"="#3291a4"))+
  geom_jitter(position=position_jitterdodge(jitter.width = 0.25), size=0.5, alpha=1, color="gray") + # , color="gray", size=0.5, alpha=1) +
  theme(axis.text = element_text(size = 10), axis.title = element_text(size = 12),legend.position="none") +
  xlab("Postnatal age [days]") +
  ylab("Bacteria Species H'") +
  theme(axis.line = element_line(colour = "black"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.border = element_blank(),
  ...
  stat_compare_means(
    aes(DOL_Group = NEC_status), label = "p.format", # p.format or p.signif
    method = "wilcox.test", label.y = max(taxa_SD_meta$ShannonDiversity_BacSpecies) * 1.05, size = 3)
plot_SD_DOL
ggsave("Box_SD_bacSpecies_DOL.pdf", plot_SD_DOL, width=7, height=6)
