...
## Phage host data
dataset_all_genus_phage = read.csv('Phage_relAbundance_genus_all.csv', header = T, row.names = 1)
...
dataset_all_genus_phage[dataset_all_genus_phage > 0] <- 1
colnames(dataset_all_genus_phage)<-paste(colnames(dataset_all_genus_phage),"vir",sep="_")
dataset_all_species_phage = read.csv('Phage_relAbundance_species_all.csv', header = T, row.names = 1)
...
dataset_all_species_phage[dataset_all_species_phage > 0] <- 1
...
dataset_all_resistome_class = read.csv('241104_NEC_ShortBRED_byclass.csv', header = T, row.names = 1) #241104_NEC_ShortBRED_byclass.csv
...
dataset_all_resistome_class[dataset_all_resistome_class > 0] <- 1
dataset_all_resistome_gene = read.csv('240831_NEC_ShortBRED_V2.csv', header = T, row.names = 1) #241104_NEC_ShortBRED_byclass.csv
...
dataset_all_resistome_gene[dataset_all_resistome_gene > 0] <- 1
...
dataset_all_pathway = read.csv('mmc12_MAG_Pathway_RA.csv', header = T, row.names = 1)
dataset_all_pathway = select(dataset_all_pathway, -NEC_onset2,-NEC_status,-DOL,-Days_before_NEC_onset,-Patient)
dataset_all_pathway[dataset_all_pathway > 0] <- 1
...
dataset_all_iRep_genus[dataset_all_iRep_genus > 0] <- 1
...
dataset_all_iRep_species[dataset_all_iRep_species > 0] <- 1
...
dataset_all_MAX_taxa[dataset_all_MAX_taxa > 0] <- 1
...
dataset_all_MAX_path[dataset_all_MAX_path > 0] <- 1
...
dataset_all_genus_bac = read.csv('240510_NEC_bacteria_genus_RA_meta4.csv', header = T, row.names = 1)
...
dataset_all_genus_bac[dataset_all_genus_bac > 0] <- 1
colnames(dataset_all_genus_bac)<-paste(colnames(dataset_all_genus_bac),"bac",sep="_")
dataset_all_species_bac = read.csv('240510_NEC_bacteria_species_RA_meta4.csv', header = T, row.names = 1)
dataset_all_meta = select(dataset_all_species_bac, Days_before_NEC_onset,Patient,NEC_status)
...
dataset_all_species_bac[dataset_all_species_bac > 0] <- 1
...
metadata_sample2 = read_excel("241103_sample_medications.xlsx")
metadata_sample  = merge(metadata_sample1, metadata_sample2, by="DNA_Sample_ID")
metadata_subject = read_excel("241104_subject_ALL_noMat.xlsx")
...
write.table(cbind('Days_before_onset','Control.Control', 'Control.Case', 'Case.Control', 'Case.Case', 
            ...
            "241127_ML_allTaxa_MTX_singledb_pred_v1.2_BI.csv", 
...
write.table(cbind('Days_before_onset','DataSet', 'Taxa', 'Feature', 'Feature.MeanImp', 'Feature.SDImp', 'Feature.NULLMeanImp', 'Feature.NULLSDImp'), 
            "241127_ML_allTaxa_MTX_singledb_VarImpSumm_v1.2_BI.csv", 
            col.names = FALSE, row.names = FALSE, sep = ",")
...
for (DBO in 0:8){
  ...
  GM.all <- select(GM_all, -Days_before_NEC_onset, -Patient, -NEC_status)
  GM.filt <- GM.all[, colSums(GM.all) > 0]
  # GM.filt.int <- round(GM.filt*10000000)
  GM.caret <- data.frame(GM.filt)
  GM.caret[,c(1:length(GM.caret))] <- data.frame(lapply(GM.caret[,c(1:length(GM.caret))], factor))
  ...
  fs.bacrest.all = colnames(dataset_all_resistome) #[2:length(colnames(dataset_all_Bacrest))]
  ...
  bactaxa.all = GM.caret[ , colnames(GM.caret) %in% c(fs.bactaxa.all, "NEC_status")]
  ...
  bacirep.all = GM.caret[ , colnames(GM.caret) %in% c(fs.bacirep.all, "NEC_status")]
  bacpath.all = GM.caret[ , colnames(GM.caret) %in% c(fs.bacpath.all, "NEC_status")]
  bacMAX.all  = GM.caret[ , colnames(GM.caret) %in% c(fs.bacMAX.all, "NEC_status")]
  ...
  # Create Data Subsets
  #  Main Text models:
  all.bactaxa <- createSubsets(bactaxa.all, all.nullvars, Taxa=bactaxa.all)
  ...
  all.bacirep <- createSubsets(bacirep.all, all.nullvars, Taxa=bacirep.all)
  all.bacpath <- createSubsets(bacpath.all, all.nullvars, Taxa=bacpath.all)
  all.bacMAX  <- createSubsets(bacMAX.all, all.nullvars, Taxa=bacMAX.all)
  ...
  varsnot2norm <- factor.vars # this doesn't matter
  ...
  # all (all biomarkers)
  rf.all_Bactaxa <- train_rf_models(all.bactaxa$Base, cv10, 'BacTaxa', 
                              varsnot2norm, FALSE)
  rf.all_Virtaxa <- train_rf_models(all.virtaxa$Base, cv10, 'VirTaxa', 
                                        varsnot2norm, FALSE)
  rf.all_Bacrest <- train_rf_models(all.bacrest$Base, cv10, 'BacRest', 
                                        varsnot2norm, FALSE)
  rf.all_Bacirep <- train_rf_models(all.bacirep$Base, cv10, 'BacIrep', 
                                    varsnot2norm, FALSE)
  rf.all_Bacpath <- train_rf_models(all.bacpath$Base, cv10, 'BacPath', 
                                    varsnot2norm, FALSE)
  rf.all_BacMAX  <- train_rf_models(all.bacMAX$Base, cv10, 'BacMTX', 
                                    varsnot2norm, FALSE)
  ...
  # Collate results: 
  all.pred.CV <- rbind(rf.all_Bactaxa$Pred.Results.CV, 
                       rf.all_Virtaxa$Pred.Results.CV, 
                       rf.all_Bacrest$Pred.Results.CV,
                       rf.all_Bacirep$Pred.Results.CV,
                       rf.all_Bacpath$Pred.Results.CV,
                       rf.all_BacMAX$Pred.Results.CV) 
  
  all.pred.Val <- rbind(rf.all_Bactaxa$Pred.Results.Val, 
                        rf.all_Virtaxa$Pred.Results.Val,
                        rf.all_Bacrest$Pred.Results.Val,
                        rf.all_Bacirep$Pred.Results.Val,
                        rf.all_Bacpath$Pred.Results.Val,
                        rf.all_BacMAX$Pred.Results.Val)
  
  rf.all_Bactaxa$Var.Importance$Dataset = "bactaxa"
  rf.all_Virtaxa$Var.Importance$Dataset = "virtaxa"
  rf.all_Bacrest$Var.Importance$Dataset = "bacrest"
  rf.all_Bacirep$Var.Importance$Dataset = "bacirep"
  rf.all_Bacpath$Var.Importance$Dataset = "bacpath"
  rf.all_BacMAX$Var.Importance$Dataset  = "bacMTX"
  all.VarImp.list <- list(rf.all_Bactaxa$Var.Importance, 
                          rf.all_Virtaxa$Var.Importance,
                          rf.all_Bacrest$Var.Importance,
                          rf.all_Bacirep$Var.Importance,
                          rf.all_Bacpath$Var.Importance,
                          rf.all_BacMAX$Var.Importance)
  ...
  rfN.all_Bactaxa <- train_rf_models(all.bactaxa$Base, cv10, 'BacTaxa', 
                             varsnot2norm, TRUE)
  ...
  rfN.all_Bacrest <- train_rf_models(all.bacrest$Base, cv10, 'BacRest', 
                                     varsnot2norm, TRUE)
  rfN.all_Bacirep <- train_rf_models(all.bacirep$Base, cv10, 'BacIrep', 
                                     varsnot2norm, TRUE)
  rfN.all_Bacpath <- train_rf_models(all.bacpath$Base, cv10, 'BacPath', 
                                     varsnot2norm, TRUE)
  rfN.all_BacMAX  <- train_rf_models(all.bacMAX$Base, cv10, 'BacMTX', 
                                     varsnot2norm, TRUE)
  
  # Collate results: 
  NULL.all.pred.CV <- rbind(rfN.all_Bactaxa$Pred.Results.CV, 
                            rfN.all_Virtaxa$Pred.Results.CV, 
                            rfN.all_Bacrest$Pred.Results.CV,
                            rfN.all_Bacirep$Pred.Results.CV,
                            rfN.all_Bacpath$Pred.Results.CV,
                            rfN.all_BacMAX$Pred.Results.CV)
  
  NULL.all.pred.Val <- rbind(rfN.all_Bactaxa$Pred.Results.Val, 
                             rfN.all_Virtaxa$Pred.Results.Val, 
                             rfN.all_Bacrest$Pred.Results.Val,
                             rfN.all_Bacirep$Pred.Results.Val,
                             rfN.all_Bacpath$Pred.Results.Val,
                             rfN.all_BacMAX$Pred.Results.Val) 
  
  rfN.all_Bactaxa$Var.Importance$Dataset = "bactaxa"
  rfN.all_Virtaxa$Var.Importance$Dataset = "virtaxa"
  rfN.all_Bacrest$Var.Importance$Dataset = "bacrest"
  rfN.all_Bacirep$Var.Importance$Dataset = "bacirep"
  rfN.all_Bacpath$Var.Importance$Dataset = "bacpath"
  rfN.all_BacMAX$Var.Importance$Dataset = "bacMAX"
  NULL.all.VarImp.list <- list(rfN.all_Bactaxa$Var.Importance, 
                               rfN.all_Virtaxa$Var.Importance, 
                               rfN.all_Bacrest$Var.Importance,
                               rfN.all_Bacirep$Var.Importance,
                               rfN.all_Bacpath$Var.Importance,
                               rfN.all_BacMAX$Var.Importance)
  ...
  # pvar_feature <- plotVarImp(VarImp.summ, 'plus BacIrep', varimp.labels)
  # pvarimp_main <- ggarrange(pvar_all, pvar_bactaxa, 
  #                           pvar_virtaxa, pvar_bacrest, nrow=2)
  ...
  #             paste("241127_ML_allTaxa_MTX_singledb_VarImpCombined_DBO", DBO, "_v1.2_BI.csv", sep = ""),
  ...
  write.table(c(DBO,varimp.tests), 
              paste("241127_ML_allTaxa_MTX_singledb_VarImpTest_DBO", DBO, "_v1.2_BI.csv", sep = ""),
              append = FALSE, col.names = TRUE, row.names = FALSE, sep = ",")
}
