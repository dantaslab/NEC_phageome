...
dataset_all_meta = select(dataset_all_species_bac, Days_before_NEC_onset,Patient,NEC_status,NEC_onset2,Birthweight,BW_group)
...
dataset_all = filter(dataset_all, Birthweight<=800)
...
write.table(cbind('Days_before_onset','Control.Control', 'Control.Case', 'Case.Control', 'Case.Case', 
            ...
            "250813_ML_allTaxa_MTX_singledb_smaller800_pred_v1.2.csv", 
...
write.table(cbind('Days_before_onset','DataSet', 'Taxa', 'Feature', 'Feature.MeanImp', 'Feature.SDImp', 'Feature.NULLMeanImp', 'Feature.NULLSDImp'), 
            "250813_ML_allTaxa_MTX_singledb_smaller800_VarImpSumm_v1.2.csv", 
            col.names = FALSE, row.names = FALSE, sep = ",")
...
for (DBO in 0:14){
  ...
  GM.all <- select(GM_all, -Days_before_NEC_onset, -Patient, -NEC_status, -NEC_onset2, -Birthweight, -BW_group)
  ...
  #             paste("250813_ML_allTaxa_MTX_singledb_smaller800_VarImpCombined_DBO", DBO, "_v1.2.csv", sep = ""),
  ...
  write.table(c(DBO,varimp.tests), 
              paste("250813_ML_allTaxa_MTX_singledb_smaller800_VarImpTest_DBO", DBO, "_v1.2.csv", sep = ""),
              append = FALSE, col.names = TRUE, row.names = FALSE, sep = ",")
}
