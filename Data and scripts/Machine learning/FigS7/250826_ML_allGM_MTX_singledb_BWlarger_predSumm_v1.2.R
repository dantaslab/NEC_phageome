...
all.pred <- all.pred.Val.larger %>% mutate(DataSet = fct_relevel(Data, model.order))
...
p_rf_pred_line 
ggsave("250826_line_Pred_allTaxa_MTX_BWlarger_singledb_DBO_v1.2.1.pdf", p_rf_pred_line , width=8, height=7)
...
ggheatmap_sig
ggsave("250826_heatmap_Pred_allTaxa_MTX_BWlarger_singledb_DBO_v1.2.1.pdf", ggheatmap_sig, width=8, height=8)
...
# scale_x_reverse(limits = c(14, 0), breaks=c(0:14))
p_rf_pred_line 
ggsave("250826_line_Pred_allTaxa_MTX_BWlarger_singledb_DBO_v1.2.2.pdf", p_rf_pred_line , width=8, height=4)
...
#                              title.position = "top", title.hjust = 0.5))
ggheatmap_sig
ggsave("250826_heatmap_Pred_allTaxa_MTX_BWlarger_singledb_DBO_v1.2.2.pdf", ggheatmap_sig, width=8, height=5)
...
##########################################################################################################################################
# 
# all.pred.Val3 = filter(all.pred.Val1, DataSet == "VirTaxa")
# 
# t_test_result_BW = c()
# for (group in unique(all.pred.Val3$Group)) {
#   for (measure in c('Balanced.Accuracy', 'Sensitivity', 'Specificity')){
#     all.pred.long_bw <- subset(all.pred.Val3, Group == group & Performance_Measure==measure)
#     t_test_result <- all.pred.long_bw %>% t_test(Performance_Measure_Value ~ BW, ref.group = "800", p.adjust.method = "BH")
#     t_test_result$Group = group
#     t_test_result$Performance_Measure = measure
#     t_test_result$mean_Performance_Measure = c(mean(all.pred.long_bw$Performance_Measure_Value[all.pred.long_bw$BW=='750']),
#                                                mean(all.pred.long_bw$Performance_Measure_Value[all.pred.long_bw$BW=='850']),
#                                                mean(all.pred.long_bw$Performance_Measure_Value[all.pred.long_bw$BW=='900']),
#                                                mean(all.pred.long_bw$Performance_Measure_Value[all.pred.long_bw$BW=='950']),
#                                                mean(all.pred.long_bw$Performance_Measure_Value[all.pred.long_bw$BW=='1000']))
#     # t_test_result$mean_Performance_Measure = mean(all.pred.long_bw$Performance_Measure_Value)
#     t_test_result_BW = rbind(t_test_result_BW,t_test_result)
#   }
# }
# t_test_result_BW$Performance_Measure <- factor(t_test_result_BW$Performance_Measure,
#                                                     levels = c("Balanced.Accuracy","Sensitivity","Specificity"))
# 
# 
# ## Generate heatmap to show the differences of dataset compared to virome
# library(rstatix)
# t_test_result_BW$group2 = factor(t_test_result_BW$group2)
# # t_test_result_BW$statistic_rev = -t_test_result_BW$statistic
# t_test_result_BW$p.adj.signif[t_test_result_BW$p.adj.signif=="ns"]=""
# ggheatmap <- ggplot(t_test_result_BW, aes(group2, Performance_Measure, fill = statistic))+
#   geom_tile(color = "black")+
#   scale_fill_gradient2(low = "#006b93", high = "#e32633", mid = "white",
#                        midpoint = 0,
#                        # limits = c(-8,17), values = rescale(c(-8,0,17)),
...
# 
# ggheatmap_sig <- ggheatmap +
#   geom_text(aes(group2,Performance_Measure), label = t_test_result_BW$p.adj.signif, color = "black", size = 4) +
#   theme(
#     # axis.title.x = element_blank(),
#     # axis.title.y = element_blank(),
#     panel.grid.major = element_blank(),
