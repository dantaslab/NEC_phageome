...
write.table(cbind('Days_before_onset','Control.Control', 'Control.Case', 'Case.Control', 'Case.Case', 
            ...
            "241129_ML_allTaxa_MAX_late_rest_pred_v1.2.csv", 
...
write.table(cbind('Days_before_onset','DataSet', 'Taxa', 'Feature', 'Feature.MeanImp', 'Feature.SDImp', 'Feature.NULLMeanImp', 'Feature.NULLSDImp'), 
            "241129_ML_allTaxa_MAX_late_rest_VarImpSumm_v1.2.csv", 
            col.names = FALSE, row.names = FALSE, sep = ",")
...
for (DBO in 0:8){
  ...
  # all (all biomarkers)
  rf.all <- train_rf_models(all.bactaxa$Base, cv10, 'BacRest', 
                                    varsnot2norm, FALSE)
  rf.all_Bactaxa <- train_rf_models(all.bactaxa$WithTaxa, cv10, 'Plus BacTaxa', 
                                    varsnot2norm, FALSE)
  rf.all_Virtaxa <- train_rf_models(all.virtaxa$WithTaxa, cv10, 'Plus VirTaxa', 
                                    varsnot2norm, FALSE)
  ...
  rfN.all <- train_rf_models(all.bactaxa$Base, cv10, 'BacRest', 
                             varsnot2norm, TRUE)
  rfN.all_Bactaxa <- train_rf_models(all.bactaxa$WithTaxa, cv10, 'Plus BacTaxa', 
                                     varsnot2norm, TRUE)
  rfN.all_Virtaxa <- train_rf_models(all.virtaxa$WithTaxa, cv10, 'Plus VirTaxa', 
                                     varsnot2norm, TRUE)
  ...
  #             paste("241129_ML_allTaxa_MAX_late_rest_VarImpCombined_DBO", DBO, "_v1.2.csv", sep = ""),
  ...
  write.table(c(DBO,varimp.tests), 
              paste("241129_ML_allTaxa_MAX_late_rest_VarImpTest_DBO", DBO, "_v1.2.csv", sep = ""),
              append = FALSE, col.names = TRUE, row.names = FALSE, sep = ",")
}
