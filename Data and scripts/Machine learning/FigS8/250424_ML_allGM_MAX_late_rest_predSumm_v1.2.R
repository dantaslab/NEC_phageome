...
## Load data
setwd('~/Google Drive/WashU/Phageome/NEC/Manuscript/Figures_drop/Figure 4_ML/Boruta/AddOn_resistome_late')
all.pred.Val2 <- read.csv('250524_ML_allTaxa_MAX_late_rest_pred_v1.2.csv', header = T)
...
model.order <- c('Plus VirTaxa', 'Plus BacTaxa', 'BacRest', 'Plus BacPath', 'Plus BacIrep', 'Plus BacMAX', 'All Features')
...
mu <- ddply(subset(all.pred.long, DataSet == "BacRest"), 
...
p_rf_pred_box <- ggplot(all.pred.long, aes(x=DataSet, y=Performance_Measure_Value, 
  ...
  #            linetype = 2)
p_rf_pred_box
ggsave("250424_box_Pred_All_AddOnBacrest_late_DBO>0_v1.2.pdf", p_rf_pred_box , width=10, height=3)
...
for (tp in 0:8) {
  for (measure in c('Balanced.Accuracy', 'Sensitivity', 'Specificity')){
    ...
    t_test_result$mean_Performance_Measure = c(mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='Plus BacTaxa']), 
                                               mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='Plus VirTaxa']),
                                               mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='Plus BacPath']),
                                               mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='Plus BacIrep']),
                                               mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='Plus BacMAX']),
                                               mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='All Features']))
...
p_rf_pred_line 
ggsave("241129_line_Pred_All_AddOnVirome_late_DBO_v1.2.pdf", p_rf_pred_line , width=12, height=3)
