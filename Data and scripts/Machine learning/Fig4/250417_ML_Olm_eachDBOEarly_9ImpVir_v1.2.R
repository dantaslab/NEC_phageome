...
## Phage host data - NEC
dataset_all_genus_phage = read.csv('Input_NEC/Phage_relAbundance_genus_all.csv', header = T, row.names = 1)
...
dataset_all_species_phage = read.csv('Input_NEC/Phage_relAbundance_species_all.csv', header = T, row.names = 1)
...
## Bacterial function data - NEC * no iRep
dataset_all_resistome_class = read.csv('Input_NEC/241104_NEC_ShortBRED_byclass.csv', header = T, row.names = 1) #241104_NEC_ShortBRED_byclass.csv
...
dataset_all_resistome_gene = read.csv('Input_NEC/240831_NEC_ShortBRED_V2.csv', header = T, row.names = 1) #241104_NEC_ShortBRED_byclass.csv
...
colnames(dataset_all_resistome)[1] = "Row.names"
...
dataset_all_pathway = read.csv('Input_NEC/mmc12_MAG_Pathway_RA.csv', header = T, row.names = 1)
...
# dataset_all_iRep_genus = read.csv('mmc15_iRep_genus.csv', header = T, row.names = 1)
...
# dataset_all_iRep_species = read.csv('mmc15_iRep_species.csv', header = T, row.names = 1)
...
## MAX data
# dataset_all_MAX_taxa = read.csv('mmc13_MAX_Species_RA.csv', header = T, row.names = 1)
...
# dataset_all_MAX_path = read.csv('mmc14_MAX_Pathway_RA.csv', header = T, row.names = 1) #241104_NEC_ShortBRED_byclass.csv
...
dataset_all_genus_bac = read.csv('Input_NEC/240510_NEC_bacteria_genus_RA_meta4.csv', header = T, row.names = 1)
...
dataset_all_species_bac = read.csv('Input_NEC/240510_NEC_bacteria_species_RA_meta4.csv', header = T, row.names = 1)
...
dataset_all  = merge(dataset_all2,dataset_all_resistome,by="Row.names")
...
metadata_sample1 = read_excel("Input_NEC/240404_sample_ALL.xlsx")
metadata_sample2 = read_excel("Input_NEC/241103_sample_medications.xlsx")
metadata_sample  = merge(metadata_sample1, metadata_sample2, by="DNA_Sample_ID")
metadata_subject = read_excel("Input_NEC/241104_subject_ALL_noMat.xlsx")
...
Boruta.df   = read.csv('Input_NEC/250415_NEC_Boruta_early_sum.csv', header = T)
Boruta.vir  = unique(Boruta.df$Feature[Boruta.df$Dataset=="VirTaxa"])

dataset_all_Olm = read.csv('Input_Olm/250208_Olm_allGM_NECstatus_match_noNA.csv', header = T, row.names = 1)
GM_all_Olm      = select(dataset_all_Olm, -NEC_status)
GM.filt.Olm     = GM_all_Olm[, which(colSums(GM_all_Olm) > 0)]
GM.filt.int.Olm =  round(GM.filt.Olm*10000000)
GM.caret.Olm    = cbind(data.frame(GM.filt.int.Olm), dataset_all_Olm[3727])
...
write.table(cbind('Days_before_onset','Control.Control', 'Control.Case', 'Case.Control', 'Case.Case', 
            ...
            "250417_ML_Olm_9vir_pred_v1.2_match_Nosplit_VarByDay.csv", 
...
write.table(cbind('Days_before_onset','DataSet', 'Taxa', 'Feature', 'Feature.MeanImp', 'Feature.SDImp', 'Feature.NULLMeanImp', 'Feature.NULLSDImp'), 
            "250417_ML_Olm_9vir_VarImpSumm_v1.2_match_Nosplit_VarByDay.csv", 
            col.names = FALSE, row.names = FALSE, sep = ",")
...
for (DBO in 0:8){
  ...
  ImpVar.df        = read.csv(paste("Input_NEC/Boruta/250212_NEC_boruta_DBO", DBO, ".csv", sep = ""), header = T)
  ImpVar.all       = unique(ImpVar.df$Var1)
  ImpVar.vir       = unique(ImpVar.df$Var1[ImpVar.df$Test_group=="VirTaxa"])
  ...
  fs.virtaxa.imp     = Boruta.vir #ImpVar.vir
  fs.all.impvari     = c('Shigella_vir', 'Enterobacter_vir', 'Salmonella_vir', 'Klebsiella_vir', 'Klebsiella_grimontii_vir', 'Atlantibacter_vir', 'Dickeya_vir', 'Serratia_vir', 'Enterococcus_faecalis_vir')
  ...
  all.impvari     = GM.caret[ , colnames(GM.caret) %in% c(fs.all.impvari, "NEC_status")]
  
  virtaxa.all.olm     = GM.caret.Olm[ , colnames(GM.caret.Olm) %in% c(fs.virtaxa.all, "NEC_status")] 
  virtaxa.imp.olm     = GM.caret.Olm[ , colnames(GM.caret.Olm) %in% c(fs.virtaxa.imp, "NEC_status")] 
  all.impvari.olm     = GM.caret.Olm[ , colnames(GM.caret.Olm) %in% c(fs.all.impvari, "NEC_status")]
  
  # maaslinBIN.all  = GM.caret[ , colnames(GM.caret) %in% c(fs.maaslinBIN.all, "NEC_status")] 
  # maaslinLM.all   = GM.caret[ , colnames(GM.caret) %in% c(fs.maaslinLM.all, "NEC_status")] 
  # maaslinBIN.dbo8 = GM.caret[ , colnames(GM.caret) %in% c(fs.maaslinBIN.dbo8, "NEC_status")] 
  # maaslinLM.dbo8  = GM.caret[ , colnames(GM.caret) %in% c(fs.maaslinLM.dbo8, "NEC_status")] 
  ...
  imp.allvari     <- createSubsets(all.impvari, all.nullvars, Taxa=all.impvari)
  
  all.virtaxa.olm     <- createSubsets(virtaxa.all.olm, all.nullvars, Taxa=virtaxa.all.olm)
  imp.virtaxa.olm     <- createSubsets(virtaxa.imp.olm, all.nullvars, Taxa=virtaxa.imp.olm)
  imp.allvari.olm     <- createSubsets(all.impvari.olm, all.nullvars, Taxa=all.impvari.olm)
  
  # all.maaslinBIN  <- createSubsets(maaslinBIN.all, all.nullvars, Taxa=maaslinBIN.all)
  # all.maaslinLM   <- createSubsets(maaslinLM.all, all.nullvars, Taxa=maaslinLM.all)
  # dbo8.maaslinBIN <- createSubsets(maaslinBIN.dbo8, all.nullvars, Taxa=maaslinBIN.dbo8)
  # dbo8.maaslinLM  <- createSubsets(maaslinLM.dbo8, all.nullvars, Taxa=maaslinLM.dbo8)
  
  ## Train Random Forest classifiers----------------------------------------------
  
  # Define the test harness. Within the training cohort, will train using 10-fold
  # cross-validation. 
  cv10 <- trainControl(method='cv', number=2, classProbs=T, savePredictions =T)
  ...
  train_rf_models <- function(data, val.data, control.harness, data.name.string, 
    ...
    for (i in 1:100) {
      ...
      # data.val <- val.data[, colnames(val.data) %in% colnames(data1)]  #data[-data1_idx, ]
      ...
      data.train <- data.train[, colnames(data.train) %in% colnames(val.data)]
      # data.test <- data1[-train_idx, ]
      
      data.val <- val.data[, colnames(data.train) %in% colnames(val.data)] #[, colnames(val.data) %in% colnames(data.train)]  #data[-data1_idx, ]
      ...
      cM.val <- confusionMatrix(predictions.val.rf, as.factor(data.val.p$NEC_status))
  ...
  rf.all_Virtaxa     <- train_rf_models(all.virtaxa$Base, GM.caret.Olm, cv10, 'VirTaxa', 
                                        varsnot2norm, FALSE)
  rf.imp_Virtaxa     <- train_rf_models(imp.virtaxa$Base, GM.caret.Olm, cv10, 'BorutaVirTaxa', 
                                        varsnot2norm, FALSE)
  rf.imp_Allvari     <- train_rf_models(imp.allvari$Base, GM.caret.Olm, cv10, 'impAllVari', 
                                        varsnot2norm, FALSE)
  # rf.all_MaaslinBIN  <- train_rf_models(all.maaslinBIN$Base, cv10, 'allMaaslinBIN', 
  #                                       varsnot2norm, FALSE)
  # rf.all_MaaslinLM   <- train_rf_models(all.maaslinLM$Base, cv10, 'allMaaslinLM', 
  #                                       varsnot2norm, FALSE)
  # rf.dbo8_MaaslinBIN <- train_rf_models(dbo8.maaslinBIN$Base, cv10, 'dbo8MaaslinBIN', 
  #                                       varsnot2norm, FALSE)
  # rf.dbo8_MaaslinLM  <- train_rf_models(dbo8.maaslinLM$Base, cv10, 'dbo8MaaslinLM', 
  #                                       varsnot2norm, FALSE)
  
  
  all.pred.Val <- rbind(rf.all_Virtaxa$Pred.Results.Val,
                        rf.imp_Virtaxa$Pred.Results.Val,
                        rf.imp_Allvari$Pred.Results.Val)
  # rf.all_MaaslinBIN$Pred.Results.Val,
  # rf.all_MaaslinLM$Pred.Results.Val,
  # rf.dbo8_MaaslinBIN$Pred.Results.Val,
  # rf.dbo8_MaaslinLM$Pred.Results.Val)
  
  rf.all_Virtaxa$Var.Importance$Dataset = "virtaxa"
  rf.imp_Virtaxa$Var.Importance$Dataset = "borutavirtaxa"
  rf.imp_Allvari$Var.Importance$Dataset = "impallvari"
  # rf.all_MaaslinBIN$Var.Importance$Dataset = "allmaaslinBIN"
  # rf.all_MaaslinLM$Var.Importance$Dataset = "allmaaslinLM"
  # rf.dbo8_MaaslinBIN$Var.Importance$Dataset = "dbo8maaslinBIN"
  # rf.dbo8_MaaslinLM$Var.Importance$Dataset = "dbo8maaslinLM"
  
  all.VarImp.list <- list(rf.all_Virtaxa$Var.Importance,
                          rf.imp_Virtaxa$Var.Importance,
                          rf.imp_Allvari$Var.Importance)
  # rf.all_MaaslinBIN$Var.Importance,
  # rf.all_MaaslinLM$Var.Importance,
  # rf.dbo8_MaaslinBIN$Var.Importance,
  # rf.dbo8_MaaslinLM$Var.Importance)
  ...
  rfN.all_Virtaxa     <- train_rf_models(all.virtaxa$Base, GM.caret.Olm, cv10, 'VirTaxa', 
                                         varsnot2norm, TRUE)
  rfN.imp_Virtaxa     <- train_rf_models(imp.virtaxa$Base, GM.caret.Olm, cv10, 'BorutaVirTaxa', 
                                         varsnot2norm, TRUE)
  rfN.imp_Allvari     <- train_rf_models(imp.allvari$Base, GM.caret.Olm, cv10, 'impAllVari', 
                                         varsnot2norm, TRUE)
  # rfN.all_MaaslinBIN  <- train_rf_models(all.maaslinBIN$Base, cv10, 'allMaaslinBIN', 
  #                                        varsnot2norm, TRUE)
  # rfN.all_MaaslinLM   <- train_rf_models(all.maaslinLM$Base, cv10, 'allMaaslinLM', 
  #                                        varsnot2norm, TRUE)
  # rfN.dbo8_MaaslinBIN <- train_rf_models(dbo8.maaslinBIN$Base, cv10, 'dbo8MaaslinBIN', 
  #                                        varsnot2norm, TRUE)
  # rfN.dbo8_MaaslinLM  <- train_rf_models(dbo8.maaslinLM$Base, cv10, 'dbo8MaaslinLM', 
  #                                        varsnot2norm, TRUE)
  
  NULL.all.pred.Val <- rbind(rfN.all_Virtaxa$Pred.Results.Val,
                             rfN.imp_Virtaxa$Pred.Results.Val,
                             rfN.imp_Allvari$Pred.Results.Val)
  # rfN.all_MaaslinBIN$Pred.Results.Val,
  # rfN.all_MaaslinLM$Pred.Results.Val,
  # rfN.dbo8_MaaslinBIN$Pred.Results.Val,
  # rfN.dbo8_MaaslinLM$Pred.Results.Val)
  
  rfN.all_Virtaxa$Var.Importance$Dataset = "virtaxa"
  rfN.imp_Virtaxa$Var.Importance$Dataset = "borutavirtaxa"
  rfN.imp_Allvari$Var.Importance$Dataset = "impallvari"
  # rfN.all_MaaslinBIN$Var.Importance$Dataset = "allmaaslinBIN"
  # rfN.all_MaaslinLM$Var.Importance$Dataset = "allmaaslinLM"
  # rfN.dbo8_MaaslinBIN$Var.Importance$Dataset = "dbo8maaslinBIN"
  # rfN.dbo8_MaaslinLM$Var.Importance$Dataset = "dbo8maaslinLM"
  
  NULL.all.VarImp.list <- list(rfN.all_Virtaxa$Var.Importance,
                               rfN.imp_Virtaxa$Var.Importance,
                               rfN.imp_Allvari$Var.Importance)
  # rfN.all_MaaslinBIN$Var.Importance,
  # rfN.all_MaaslinLM$Var.Importance,
  # rfN.dbo8_MaaslinBIN$Var.Importance,
  # rfN.dbo8_MaaslinLM$Var.Importance)
  ...
  # pvar_allmaaslinBIN <- plotVarImp(VarImp.summ, 'plus allMaaslinBIN', varimp.labels)
  # pvar_feature <- plotVarImp(VarImp.summ, 'plus impAllVari', varimp.labels)
  # pvarimp_main <- ggarrange(pvar_all, pvar_bactaxa, 
  #                           pvar_virtaxa, pvar_allmaaslinBIN, nrow=2)
  ...
  #             paste("241202_ML_Maaslin_both_VarImpCombined_DBO", DBO, "_v1.2.csv", sep = ""),
  ...
  varimp.tests <- data.frame(matrix(ncol=5, nrow=5000))
  ...
  write.table(c(DBO,varimp.tests), 
              paste("250417_ML_Olm_9vir_VarImpTest_DBO", DBO, "_v1.2_match_Nosplit_VarByDay.csv", sep = ""),
              append = FALSE, col.names = TRUE, row.names = FALSE, sep = ",")
