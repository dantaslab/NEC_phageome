...
## Load data
VarImp.summ <- read.csv('250416_ML_ImpVIr_early_VarImpSumm_v1.2.csv', header = T)
...
pvar_meta <- plotVarImp(VarImp.summ_DBO, 'impAllVari9', varimp.labels)
pvar_meta
# when DBO=0
...
for (DBO in 0:8){
  ...
  # for (dataset in unique(VarImp.summ$DataSet)[6]){
    varimp.tests <- read.csv(paste("250416_ML_ImpVIr_early_VarImpTest_DBO",DBO,"_v1.2.csv", sep=""), header = T)
    ...
    VarImp.summ.sub = filter(VarImp.summ, Feature %in% unique(varimp.tests$Feature) & Days_before_onset==DBO & DataSet == dataset)
    ...
    plot
    ggsave(paste("250416_bar_", dataset,"_ImpVir_early_DBO",DBO ,"_v1.2.pdf", sep=""), plot , width=4, height=5)
  # }
}
