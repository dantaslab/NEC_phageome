...
for (DBO in 0:8){
  ...
  fs.all.virtaxa <- runFeatureSelection(all.bactaxa$Base, 1:100)
  fs.all.bactaxa <- runFeatureSelection(all.bactaxa$WithTaxa, 1:100)
  fs.all.bacrest <- runFeatureSelection(all.bacrest$WithTaxa, 1:100)
  ...
  # GM.NEC.virtaxa.boruta <- subset(GM.caret, select = c(as.matrix(fs.all.top$Var1), "NEC_status"))
  # GM.NEC.virtaxa.boruta.df = gather(GM.NEC.virtaxa.boruta, "Feature", "Abundance", -NEC_status)

  write.table(c(DBO,fs.all.top), 
              paste("250212_NEC_boruta_early_DBO", DBO, ".csv", sep = ""),
