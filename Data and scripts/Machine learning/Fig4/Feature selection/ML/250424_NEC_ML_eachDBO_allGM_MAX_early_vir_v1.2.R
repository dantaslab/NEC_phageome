...
write.table(cbind('Days_before_onset','Control.Control', 'Control.Case', 'Case.Control', 'Case.Case', 
            ...
            "250424_ML_allTaxa_MAX_early_pred_v1.2.csv", 
...
write.table(cbind('Days_before_onset','DataSet', 'Taxa', 'Feature', 'Feature.MeanImp', 'Feature.SDImp', 'Feature.NULLMeanImp', 'Feature.NULLSDImp'), 
            "250424_ML_allTaxa_MAX_early_VarImpSumm_v1.2.csv", 
            col.names = FALSE, row.names = FALSE, sep = ",")
...
for (DBO in 0:8){
  ...
  bactaxa.boruta = GM.caret[ , colnames(GM.caret) %in% fs.bactaxa.all]
  virtaxa.boruta = GM.caret[ , colnames(GM.caret) %in% c(fs.virtaxa.all, "NEC_status")] 
  base.df = virtaxa.boruta
  bacrest.all = GM.caret[ , colnames(GM.caret) %in% fs.bacrest.all] 
  ...
  feature.all = GM.caret[ , !colnames(GM.caret) %in% c(fs.virtaxa.all, "NEC_status")] 
  ...
  # Create Data Subsets
  #  Main Text models:
  all.bactaxa <- createSubsets(base.df, all.nullvars, Taxa=bactaxa.boruta)
  # all.virtaxa <- createSubsets(base.df, all.nullvars, Taxa=virtaxa.boruta)
  ...
  # all (all biomarkers)
  rf.all <- train_rf_models(all.bactaxa$Base, cv10, 'VirTaxa', 
                                    varsnot2norm, FALSE)
  rf.all_Bactaxa <- train_rf_models(all.bactaxa$WithTaxa, cv10, 'Plus BacTaxa', 
                                    varsnot2norm, FALSE)
  rf.all_Bacrest <- train_rf_models(all.bacrest$WithTaxa, cv10, 'Plus BacRest', 
                                    varsnot2norm, FALSE)
  rf.all_Bacirep <- train_rf_models(all.bacirep$WithTaxa, cv10, 'Plus BacIrep', 
                                    varsnot2norm, FALSE)
  rf.all_Bacpath <- train_rf_models(all.bacpath$WithTaxa, cv10, 'Plus BacPath', 
                                    varsnot2norm, FALSE)
  rf.all_BacMAX  <- train_rf_models(all.bacMAX$WithTaxa, cv10, 'Plus BacMAX', 
                                    varsnot2norm, FALSE)
  rf.all_Feature <- train_rf_models(all.feature$WithTaxa, cv10, 'All Features', 
                                    varsnot2norm, FALSE)
  ...
  # Collate results: 
  # all.pred.CV <- rbind(rf.all$Pred.Results.CV, 
  #                      rf.all_Bactaxa$Pred.Results.CV, 
  #                      rf.all_Bacrest$Pred.Results.CV,
  #                      rf.all_Bacirep$Pred.Results.CV,
  #                      rf.all_Bacpath$Pred.Results.CV,
  #                      rf.all_BacMAX$Pred.Results.CV,
  #                      rf.all_Feature$Pred.Results.CV) 
  # 
  all.pred.Val <- rbind(rf.all$Pred.Results.Val, 
                        rf.all_Bactaxa$Pred.Results.Val,
                        ...
                        rf.all_Feature$Pred.Results.Val)
  
  all.VarImp.list <- list(rf.all$Var.Importance, 
                          rf.all_Bactaxa$Var.Importance,
                          rf.all_Bacrest$Var.Importance,
                          rf.all_Bacirep$Var.Importance,
                          rf.all_Bacpath$Var.Importance,
                          rf.all_BacMAX$Var.Importance,
                          rf.all_Feature$Var.Importance)
  ...
  rfN.all <- train_rf_models(all.bactaxa$Base, cv10, 'VirTaxa', 
                             varsnot2norm, TRUE)
  rfN.all_Bactaxa <- train_rf_models(all.bactaxa$WithTaxa, cv10, 'Plus BacTaxa', 
                                     varsnot2norm, TRUE)
  rfN.all_Bacrest <- train_rf_models(all.bacrest$WithTaxa, cv10, 'Plus BacRest', 
                                     varsnot2norm, TRUE)
  rfN.all_Bacirep <- train_rf_models(all.bacirep$WithTaxa, cv10, 'Plus BacIrep', 
                                     varsnot2norm, TRUE)
  rfN.all_Bacpath <- train_rf_models(all.bacpath$WithTaxa, cv10, 'Plus BacPath', 
                                     varsnot2norm, TRUE)
  rfN.all_BacMAX  <- train_rf_models(all.bacMAX$WithTaxa, cv10, 'Plus BacMAX', 
                                     varsnot2norm, TRUE)
  rfN.all_Feature <- train_rf_models(all.feature$WithTaxa, cv10, 'All Features', 
                                     varsnot2norm, TRUE)
  
  
  
  # Collate results: 
  # NULL.all.pred.CV <- rbind(rfN.all$Pred.Results.CV, 
  #                           rfN.all_Bactaxa$Pred.Results.CV, 
  ...
  #                           rfN.all_Feature$Pred.Results.CV) 
  
  NULL.all.pred.Val <- rbind(rfN.all$Pred.Results.CV, 
                             rfN.all_Bactaxa$Pred.Results.CV, 
                             rfN.all_Bacrest$Pred.Results.CV,
                             rfN.all_Bacirep$Pred.Results.CV,
                             rfN.all_Bacpath$Pred.Results.CV,
  ...
  #             paste("250424_ML_allTaxa_MAX_early_VarImpCombined_DBO", DBO, "_v1.2.csv", sep = ""),
  ...
  varimp.tests <- data.frame(matrix(ncol=5, nrow=7000))
  ...
  write.table(c(DBO,varimp.tests), 
              paste("250424_ML_allTaxa_MAX_early_VarImpTest_DBO", DBO, "_v1.2.csv", sep = ""),
              append = FALSE, col.names = TRUE, row.names = FALSE, sep = ",")
}
