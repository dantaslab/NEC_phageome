...
## Load data
VarImp.summ <- read.csv('250418_ML_9ImpVir_meta_early_VarImpSumm_v1.2.csv', header = T)
...
# 
# # Main text models
unique(VarImp.summ$DataSet) #"imp9Vir" "imp9Vir_Allmeta" "imp9Vir_impmeta.all1"   "imp9Vir_impmeta.all2"   "imp9Vir_impmeta.early1" "imp9Vir_impmeta.early2"
DBO=0
VarImp.summ_DBO <- filter(VarImp.summ, Days_before_onset==DBO)
pvar_meta <- plotVarImp(VarImp.summ_DBO, 'imp9Vir_impmeta.early2', varimp.labels)
pvar_meta
# when DBO=0
# A = reorder(VarImp.summ.sub$Feature, VarImp.summ.sub$Feature.MeanImp)
early.keymetadata1 = c('Calcium_gluconate', 'Chlorathiazide', 'Iron', 'milk_cumulative', 'Abx_cumulative', 
                       'ANTIM_cumulative', 'GA_birth', 'Lasix', 'SexMale', '3rd_gen_cephalosporin', 'route')
early.keymetadata2 = c('Calcium_gluconate', 'Chlorathiazide', 'Iron', 'milk_cumulative', 'Abx_cumulative', 
                       'ANTIM_cumulative', 'GA_birth', 'Lasix', 'SexMale', '3rd_gen_cephalosporin', 'route', 'Birthweight', 'Insulin')
...
plotVarImp <- function(VarImp.df, Data.Set, var.labels){
  ...
  p_varimp <- ggplot(VarImp.df, #subset(VarImp.df, DataSet== Data.Set & !is.na(Feature.MeanImp)),
                     aes(x=Feature, y=Feature.MeanImp))+
    geom_col(fill='#66b2b2',width=0.75)+
    geom_point(color='black')+
    geom_errorbar(aes(ymin = Feature.MeanImp - Feature.SDImp,
                      ymax = Feature.MeanImp + Feature.SDImp), width=0.5, size=0.8)+
    geom_point(aes(x=Feature, y=Feature.NULLMeanImp),
               color='pink', alpha=1,shape=18,size=4)+
    geom_errorbar(aes(ymin = Feature.NULLMeanImp - Feature.NULLSDImp,
                      ymax = Feature.NULLMeanImp + Feature.NULLSDImp), width=0.5, size=0.6,
                  color='pink', alpha=0.80)+
    ...
    ylim(0, 7) +
    scale_x_discrete(limits = rev(early.keymetadata2))+
    # scale_x_discrete(labels = var.labels)+
    coord_flip()
}

# write.table(cbind('Days_before_onset','DataSet', 'Feature', 'P.val', 'Mean.Emp', 'Mean.Null', 'Diff', 'P.val.adj', 'Adj_Stars'), 
#             "250223_ML_allmeta_VarImpTest2_v1.2.csv", 
#             col.names = FALSE, row.names = FALSE, sep = ",")

for (DBO in 0:8){
  dataset = "imp9Vir_impmeta.early2"
  # for (dataset in unique(VarImp.summ$DataSet)[6]){
    varimp.tests <- read.csv(paste("250418_ML_9ImpVir_meta_early_VarImpTest_DBO",DBO,"_v1.2.csv", sep=""), header = T)
    ...
    varimp.tests2 <- varimp.tests2 %>%
      ...
      #   Feature
      # )) %>%
      filter(Feature %in% early.keymetadata2) %>%
    ...
    VarImp.summ.sub = filter(VarImp.summ, Feature %in% early.keymetadata2 & Days_before_onset==DBO & DataSet == dataset)
    pvar.sub <- plotVarImp(VarImp.summ.sub, dataset, unique(VarImp.summ.sub$Feature))
    pvar.sub
    plot = pvar.sub + geom_text(data = varimp.tests2, aes(x = Feature, y = 0.2, label = Adj_Stars), size = 7, vjust = 0.75, color = "white")
    plot
    ggsave(paste("250418_bar_", dataset,"_ImpMeta_early_DBO",DBO ,"_v1.2.pdf", sep=""), plot , width=4, height=5)
  # }
}

# meta_VarImp_df = read.csv("250223_ML_allmeta_VarImpTest2_v1.2.csv",header = 1)
