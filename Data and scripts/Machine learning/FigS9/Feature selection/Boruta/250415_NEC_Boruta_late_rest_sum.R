...
write.table(cbind('Days_before_onset','Feature','Freq','Dataset'), 
            "250415_NEC_Boruta_late_rest_sum2.csv", 
            col.names = FALSE, row.names = FALSE, sep = ",")

for (DBO in 0:8){
# Load the information of the important variables
ImpVar.df        = read.csv(paste("250415_NEC_boruta_late_rest_DBO", DBO, ".csv", sep = ""), header = T)

write.table(c(ImpVar.df), 
            paste("250415_NEC_Boruta_late_rest_sum2.csv", sep = ""),
