...
all.pred.Val2 <- read.csv('250418_ML_BorutaRest_meta_late_pred_v1.2.csv', header = T)
unique(all.pred.Val2$DataSet)
all.pred.Val2 <- filter(all.pred.Val2, DataSet == "borutaRest" | DataSet == "borutaRest_Allmeta" | DataSet == "borutaRest_impmeta.late1")
...
model.order <- c('borutaRest', 'borutaRest_Allmeta', 'borutaRest_impmeta.late1')
...
for (tp in 0:8) {
  for (measure in c('Balanced.Accuracy', 'Sensitivity', 'Specificity')){
    ...
    t_test_result <- all.pred.long_tp %>% t_test(Performance_Measure_Value ~ DataSet, ref.group = "borutaRest", p.adjust.method = "BH")
    t_test_result$Days_before_onset = tp
    t_test_result$Performance_Measure = measure
    t_test_result$mean_Performance_Measure = c(mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='borutaRest_Allmeta']), 
                                               mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='borutaRest_impmeta.all1']),
                                               mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='borutaRest_impmeta.all2']),
                                               mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='borutaRest_impmeta.late1']),
                                               mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='borutaRest_impmeta.late2']))
...
p_rf_pred_line 
ggsave("250418_line_Pred_BorutaRest_meta_late_DBO_v1.2.pdf", p_rf_pred_line , width=10, height=3)

##----------------------------------------------------------------------------------------------------
all.pred.Val3 = filter(all.pred.Val2, Days_before_onset>0 & Days_before_onset<=8)
...
all.pred.long$Performance_Measure_Value <- as.numeric(all.pred.long$Performance_Measure_Value)
library(plyr)
mu <- ddply(subset(all.pred.long, DataSet == "borutaRest_impmeta.late1"), 
...
p_rf_pred_box <- ggplot(all.pred.long, aes(x=DataSet, y=Performance_Measure_Value, 
  ...
  theme(axis.text.y = element_text(size=10),
        ...
        panel.border = element_rect(fill=NA, colour="black", linewidth=0.5))+
  geom_signif(comparisons = list(c('borutaRest','borutaRest_Allmeta'),c('borutaRest_Allmeta','borutaRest_impmeta.late1'),c('borutaRest','borutaRest_impmeta.late1')),
              map_signif_level = T, textsize=3, step_increase = .04) +
  # stat_compare_means(aes(group = DataSet), label = "p.signif", ref.group = "borutaRest_impmeta.late1", angle = 0) +
  ylim(0,NA) +
  coord_flip()

p_rf_pred_box

ggsave("250418_box_Pred_BorutaRest_meta_late_DBO>0_v1.2_refine.pdf", p_rf_pred_box , width=10, height=2)
...
p_rf_pred_box <- ggplot(all.pred.long, aes(x=DataSet, y=Performance_Measure_Value, 
  ...
  geom_signif(comparisons = list(c('borutaRest','borutaRest_Allmeta'),c('borutaRest','imp8Vir'),c('imp8Vir','borutaRest_impmeta.all2'),c('borutaRest_impmeta.all2','borutaRest_impmeta.late1')),
              map_signif_level = T, textsize=3, step_increase = .1) +
...
p_rf_pred_box
ggsave("250418_box_Pred_BorutaRest_meta_late_DBO>0_v1.2.pdf", p_rf_pred_box , width=10, height=3)
