...
all.pred.Val2 = read.csv('250424_ML_allTaxa_MTX_singledb_late_pred_v1.2.csv', header = T)

all.pred.Val2 = filter(all.pred.Val2, Days_before_onset>0 & Days_before_onset<=8)
...
mu <- ddply(subset(all.pred.long, DataSet == "BacRest"), #VirTaxa
...
p_rf_pred_box <- ggplot(all.pred.long, aes(x=DataSet, y=Performance_Measure_Value, fill=DataSet))+
  ...
  #             map_signif_level = T, textsize=3, step_increase = .1) +
  stat_compare_means(aes(group = DataSet), label = "p.signif", ref.group = "BacRest", angle = 0) +
...
p_rf_pred_box
ggsave("250424_box_Pred_allTaxa_MTX_late_singledb_DBO>0_v1.2_rest.pdf", p_rf_pred_box , width=10, height=3)
...
for (tp in 0:8) {
  for (measure in c('Balanced.Accuracy', 'Sensitivity', 'Specificity')){
    ...
    t_test_result$mean_Performance_Measure = c(mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='VirTaxa']),
                                               mean(all.pred.long_tp$Performance_Measure_Value[all.pred.long_tp$DataSet=='BacTaxa']),
...
p_rf_pred_line 
ggsave("250424_line_Pred_allTaxa_MTX_late_singledb_DBO_v1.2.pdf", p_rf_pred_line , width=12, height=3)

## Generate heatmap to show the differences of dataset compared to virome
library(rstatix)
t_test_result_combine$Days_before_onset = factor(t_test_result_combine$Days_before_onset)
...
ggheatmap <- ggplot(t_test_result_combine, aes(Days_before_onset, group2, fill = statistic_rev))+
...
ggheatmap_sig <- ggheatmap + 
  geom_text(aes(Days_before_onset, group2), label = t_test_result_combine$p.adj.signif, color = "black", size = 4) +
  theme(
    # axis.title.x = element_blank(),
    # axis.title.y = element_blank(),
...
ggheatmap_sig
ggsave("250424_heatmap_Pred_allTaxa_MTX_late_singledb_DBO_v1.2.pdf.pdf", ggheatmap_sig, width=12, height=3)

all.pred.long.vir = filter(all.pred.long, DataSet == "BacRest")
p_rf_pred_line_vir <- ggplot(all.pred.long.vir, aes(x=Days_before_onset, y=Performance_Measure_Value, 
                                                    fill=DataSet, color=DataSet))+
  ...
  facet_grid(DataSet~Performance_Measure,scales="fixed")+
  # facet_grid(Data~Performance_Measure)+
  scale_fill_manual(values = fillcols[3])+
  scale_color_manual(values = fillcols[3])+
  xlab("Days before NEC onset") +
  theme_classic()+
  theme(axis.text.y = element_text(size=10),
        axis.text.x = element_text(size=10),
  ...
  scale_x_reverse(limits = c(8, 0), breaks=c(0:8))
p_rf_pred_line_vir
ggsave("250424_lineVir_Pred_allTaxa_MTX_late_singledb_DBO_v1.2.pdf", p_rf_pred_line_vir , width=10, height=3)
