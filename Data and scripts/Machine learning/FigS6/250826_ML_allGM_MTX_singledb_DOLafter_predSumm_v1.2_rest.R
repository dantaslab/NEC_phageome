...
all.pred <- all.pred.Val.after %>% mutate(DataSet = fct_relevel(Data, model.order))
...
for (group in unique(all.pred.Val1$Group)) {
  for (dol in unique(all.pred.Val1$DOL)) {
    for (measure in c('Balanced.Accuracy', 'Sensitivity', 'Specificity')){
      ...
      t_test_result <- all.pred.long_dol %>% t_test(Performance_Measure_Value ~ DataSet, ref.group = "BacRest", p.adjust.method = "BH")
      t_test_result$Group = group
      t_test_result$DOL = dol
      t_test_result$Performance_Measure = measure
      t_test_result$mean_Performance_Measure = c(mean(all.pred.long_dol$Performance_Measure_Value[all.pred.long_dol$DataSet=='VirTaxa']),
                                                 mean(all.pred.long_dol$Performance_Measure_Value[all.pred.long_dol$DataSet=='BacTaxa']),
...
p_rf_pred_line 
ggsave("250826_line_Pred_allTaxa_MTX_DOLafter_singledb_DBO_v1.2.1_refine_rest.pdf", p_rf_pred_line , width=8, height=7)
...
ggheatmap_sig
ggsave("250826_heatmap_Pred_allTaxa_MTX_DOLafter_singledb_DBO_v1.2.1_refine_rest.pdf", ggheatmap_sig, width=8, height=8)
...
all.pred.Val2$Group[all.pred.Val2$Days_before_onset<=8] = "DBO 1-7"
...
# scale_x_reverse(limits = c(14, 0), breaks=c(0:14))
p_rf_pred_line 
ggsave("250826_line_Pred_allTaxa_MTX_DOLafter_singledb_DBO_v1.2.2_refine_rest.pdf", p_rf_pred_line , width=8, height=4)
...
#                              title.position = "top", title.hjust = 0.5))
ggheatmap_sig
ggsave("250826_heatmap_Pred_allTaxa_MTX_DOLafter_singledb_DBO_v1.2.2_refine_rest.pdf", ggheatmap_sig, width=8, height=5)
