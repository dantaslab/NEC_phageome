...
dataset_all_meta = select(dataset_all_species_bac, Days_before_NEC_onset,Patient,NEC_status,NEC_onset2,Birthweight,BW_group,DOL)
dataset_all_species_bac = select(dataset_all_species_bac, -NEC_onset2,-NEC_status,-DOL,-Days_before_NEC_onset,-Patient,-Birthweight,-BW_group)
...
dataset_all = filter(dataset_all, Days_before_NEC_onset+DOL<=30)
...
write.table(cbind('Days_before_onset','Control.Control', 'Control.Case', 'Case.Control', 'Case.Case', 
            ...
            "250815_ML_allTaxa_MTX_singledb_DOLbefore30_pred_v1.2.csv", 
...
write.table(cbind('Days_before_onset','DataSet', 'Taxa', 'Feature', 'Feature.MeanImp', 'Feature.SDImp', 'Feature.NULLMeanImp', 'Feature.NULLSDImp'), 
            "250815_ML_allTaxa_MTX_singledb_DOLbefore30_VarImpSumm_v1.2.csv", 
            col.names = FALSE, row.names = FALSE, sep = ",")
...
for (DBO in 0:14){
  ...
  GM.all <- select(GM_all, -Days_before_NEC_onset, -Patient, -NEC_status, -NEC_onset2, -Birthweight, -BW_group, -DOL)
  ...
  #             paste("250815_ML_allTaxa_MTX_singledb_DOLbefore30_VarImpCombined_DBO", DBO, "_v1.2.csv", sep = ""),
  ...
  write.table(c(DBO,varimp.tests), 
              paste("250815_ML_allTaxa_MTX_singledb_DOLbefore30_VarImpTest_DBO", DBO, "_v1.2.csv", sep = ""),
              append = FALSE, col.names = TRUE, row.names = FALSE, sep = ",")
}
