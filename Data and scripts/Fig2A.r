...
SM_DF <- read.csv('Phage_relAb_vOTU.csv',
                  header = T,
                  row.names = 1)
...
colnames(taxa_rich)[1] <- "Richness_vOTU"
...
plot_richness_DOL <- ggplot(taxa_rich_meta, aes(x=DOL_Group, y=Richness_vOTU, fill=NEC_status)) +
  geom_boxplot() +
  ...
  theme(axis.text = element_text(size = 10), axis.title = element_text(size = 12),legend.position=c(0.15,0.8)) +
  ...
  stat_compare_means(
    ...
    method = "wilcox.test", label.y = max(taxa_rich_meta$Richness_vOTU) * 1.05, size = 3)
plot_richness_DOL
ggsave("Box_Richness_vOTU_DOL.pdf", plot_richness_DOL, width=8, height=4)


##--------------------------------------------------------------------------------------------------------
## Calculate and compare SD
taxa_SD <- as.data.frame(diversity(SM_DF, index="shannon"))
colnames(taxa_SD)[1] <- "ShannonDiversity_vOTU"
...
plot_SD_DOL <- ggplot(taxa_SD_meta, aes(x=DOL_Group, y=ShannonDiversity_vOTU, fill=NEC_status)) +
  geom_boxplot() +
  scale_fill_manual(values=c("case"="#f28a5f","control"="#3291a4"))+
  geom_jitter(position=position_jitterdodge(jitter.width = 0.25), size=0.5, alpha=1, color="gray") + 
  ...
  stat_compare_means(
    aes(DOL_Group = NEC_status), label = "p.format", # p.format or p.signif
    method = "wilcox.test", label.y = max(taxa_SD_meta$ShannonDiversity_vOTU) * 1.05, size = 3)
plot_SD_DOL
ggsave("Box_SD_vOTU_DOL.pdf", plot_SD_DOL, width=8, height=4)
