...
write.table(cbind("Dataset","Days_before_onset","sample#","infant#","Df","SumOfSqs","R2","F","Pr(>F)"), 
            "result/PERMANOVA_result_eachDBO_NEC_late.csv", 
...
for (i in 1:5){
  ...
  for (DBO in 0:14){ # DBO: Day_before_onset 
    ...
    metadata<-metadata[,-1]
    metadata = filter(metadata,NEC_onset-DOL==DBO & NEC_onset>40) # late-onset cohort
    ...
    write.table(c(output_group[i],DBO,dim(NEC_DF)[1],dim(subject_data)[1],PERMANOVA_result[1,]), "result/PERMANOVA_result_eachDBO_NEC_late.csv", 
