...
data_mec = read_excel("~/Google Drive/WashU/Phageome/NEC/Manuscript/Figures_drop/Figure 5/9ImpVir/250420_NEC_phage_info.xlsx", sheet="All_drop")
sample_virmec = as.data.frame.array(table(data_mec$Sample, data_mec$mec_9impVir))

metadata = read_excel("virMec_phage_info.xlsx", sheet="virMec_sample")
metadata = select(metadata, Sample, DOL, Days_before_NEC_onset, Patient, NEC_onset2, NEC_status)
metadata = as.data.frame(metadata)
rownames(metadata) = metadata$Sample

metadata$Sample = NULL

sample_virmec = merge(sample_virmec, metadata, by = "row.names")
sample_virmec$virMec_gene[sample_virmec$not_9impVir>0] = "Yes"
sample_virmec$virMec_gene[sample_virmec$not_9impVir==0] = "No"

data_mec = sample_virmec
...
plot_box
ggsave("250623_box_compare_non9vir_mec_prevalence_NECstatus_early_DBO_v3.pdf", plot_box, width=4, height=4)
