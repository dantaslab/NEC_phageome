...
SM_DF_all = read.csv('NEC_bacteria_species_RA.csv',
                     header = T)
colnames(SM_DF_all)[1] = "SampleID"

k <- rowSums(SM_DF_all[,2:543]) > 0
SM_DF_all <- SM_DF_all[k,]
RowName <- SM_DF_all$SampleID

taxmat <- as.data.frame(colnames(SM_DF_all))
colnames(taxmat)[1] <- "Bac_Species"
...
setdiff(rownames(t_SM_DF_all),taxmat$Bac_Species)

rownames(taxmat) <- taxmat$Bac_Species
...
write.csv(BC_between_case_control_DOL,"BC_phyloseq_bacSpecies_between_all_DOL.csv")
