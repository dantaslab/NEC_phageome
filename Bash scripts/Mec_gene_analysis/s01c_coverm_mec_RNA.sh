...
#===============================================================================
#
# File Name    : s01c_coverm_mec_RNA.sh
# Description  : This script will align reads to a ref using bowtie2
# Usage        : sbatch s01c_coverm_mec_RNA.sh
...
sample=`sed -n ${SLURM_ARRAY_TASK_ID}p ${basedir}/241102_RNA_sample_list_2011.txt`
#ref=`sed -n ${SLURM_ARRAY_TASK_ID}p ${basedir}/Bowtie_All_phage_RNA/241102_RNA_ref_list_2011.txt`
indir="${basedir}/mec/Bowtie_mec_RNA/BowtieMapping/${sample}"
outdir="${basedir}/mec/Bowtie_mec_RNA/CoverM_contig"

#make output directory
mkdir -p ${outdir}

time coverm contig --bam-files ${indir}/${sample}_sorted.bam -m mean trimmed_mean covered_bases variance length count reads_per_base rpkm tpm --min-read-percent-identity 95 -o ${outdir}/${sample}_coverm_RNA_output.tsv 
